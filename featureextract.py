# -*- coding: utf-8 -*-
"""
Created on Sun Jul  7 00:16:31 2019
...
fr=open('g25.txt','rt')   # date file of person whose features to train
info=fr.readlines() 
fw=open('test.csv','wt')
...
df=pd.read_csv('test.csv',header=None,names=columns)
if len(df.columns)>6:
    for i in range(len(df.columns)-1,5,-1):
        df=df.drop(df.columns[[i]],axis=1)
df.columns=columns;
print(len(df))
del_rows=[37,38,39,40,165,161,163,162,164,160]
for i in del_rows:
    df=df[df.Key_Code!=i]
df=df[df.Alt==False]
...
new_final_features_median.to_csv('final_median2.csv',index=False)
...
new_final_features_mean.to_csv('testnew.csv',index=False)